...
Also irgendjemand ändert diese APC Library.

Mein Service ist betroffen.

Ich möchte jetzt gerne mit dem Service live gehen.

Geht nicht, weil der ist in einer Kohorte, die eben noch nicht durch den APC Service, durch diese Änderung halt durch ist und halt deployed worden ist.
...
Das heißt also ich kann, mein Service ist noch nicht deployed, der Service von den Leuten um mich herum ist deployed mit der neuen APC-Library.
...
Das einzige konkrete Beispiel für eine übergreifende Änderung ist eine APC Service Änderung.
...
Also wenn man jetzt sagt, ich mache eine Änderung auf eine APC Service, muss ich das auf die APC Library zeitnah deployen.
...
Aber nur eine APC Library, die vielleicht irgendwo irgendeinen Corner Case jetzt irgendwie anders löst, muss ich das halt wirklich deployen.
...
Also das hat bestimmte technische Commits, zum Beispiel Änderungen an einer APC Library.
...
Also die neue APC Library interessiert halt niemanden.
...
Also irgendwelche Fachlichkeiten, das sind halt die Sachen, die Kunden typischerweise beeindrucken und nicht die APC Library, wenn sie nicht irgendwie kundenwirksame Features hat.
...
Also letztendlich bedeutet es halt, wenn die da drüben die APC Library ändern, dann ist mein Service blockiert und ich kann meine eigenen Features halt erst mal nicht ausliefern.
...
Also deploy jetzt irgendeinen Service, da ist der neue APC-Library.

Die APC-Library hat ein Problem, das eben dazu führt, dass in der Produktion irgendetwas nicht funktioniert.
...
Also APC-Libraries geändert, ich habe das Ding in Produktion, es ist also entsperrt, also mache ich jetzt eine Änderung.

Die Änderung umfasst halt irgendwelche Dinge.

Jetzt habe ich also zwei Änderungen, die APC-Library und meine Änderungen vielleicht an den Features.

Jetzt möchte ich ja zurückrollen, nehme ich dann irgendwie diese APC-Änderung raus, irgendwie auch doof, nicht?
...
Und da kann ich jetzt irgendwie auch eine Abwägung treffen und kann halt sagen, okay, es ist jetzt erstmal wichtiger, dass wir dieses Feature schippen und das halt irgendwie rausbekommen und dann kümmere ich mich später um diesen Änderungen, wo halt irgendwie die APC-Library geändert werden muss.
