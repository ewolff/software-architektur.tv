# Folge 310 - How AI Succeeds — Insights from Manufacturing Applications with Nikita Golovko Welcome everybody to another episode of Software-Architektur im Stream.
...
It seems to be the same, sort of the same thing in a different area of application for LNMs.
...
So, how are LNMs different from Gen-AI, and how do you use them in industrial applications?

I already said that LNM is just a subset of generative AI.

Generative AI is mainly responsible for generating new content, not a new knowledge, and from an industrial perspective, generative AI can be definitely used for generating new data, synthetic data, LNMs for reasoning, LNMs for helping end users to make a decision to analyze a big amount of data.

But the crucial point is, as I already said, that LNMs part and all this, let's say, most probabilistic part of your system should be deleted from the control loop where you're affecting the physical reality around human beings or your production.
...
In classical, or more or less classical, use cases of AI and ML, it can be modeled, deployed, or provided by a vendor in an ONIX format.

It's some kind of standard format.

It's a file.

You can use different ONIX models from different vendors, and what you need to do at the end of the day is just to replace one file with another file, but the rest of the ports and adapters will stay the same.
