...
# Manually curated list of all real guests from the extraction
# Based on the previous analysis, cleaned of non-person entries
real_guests_data = {
    "Aminata Sidibe": [
...
def generate_linkedin_url(name):
    # Convert name to linkedin format
...
def extract_tags_from_episodes(episodes):
    ...
    if 'KI' in all_titles or 'AI' in all_titles or 'ChatGPT' in all_titles or 'LLM' in all_titles:
...
# VOLLSTÄNDIGE Liste aller Gäste aus allen 89 Episoden mit Gästen
# Extrahiert und bereinigt - nur echte Personen
# Anzahl Gäste: """ + str(len(real_guests_data)) + """

guests:
"""

for guest_name in sorted(real_guests_data.keys()):
    episodes = real_guests_data[guest_name]
    linkedin_url = generate_linkedin_url(guest_name)
    tags = extract_tags_from_episodes(episodes)
    
    # Generate bio based on topics
    if 'Security' in str(episodes):
        bio = "Security Experte/Expertin"
    elif 'Frontend' in str(episodes):
        bio = "Frontend-Entwicklung und -Architektur"
    elif 'Domain' in str(episodes) or 'DDD' in str(episodes):
        bio = "Domain-driven Design Experte/Expertin"
    elif 'API' in str(episodes):
        bio = "API Design und Architektur"
    elif 'Testing' in str(episodes) or 'Quality' in str(episodes):
        bio = "Testing und Quality Assurance"
        ...
        bio = "KI und Software-Architektur"
    elif 'Cloud' in str(episodes):
        bio = "Cloud Computing und Architektur"
    elif 'Diversity' in str(episodes):
        bio = "Diversity & Inclusion Expertin"
    else:
...
# Replace the current guests.yml with the complete version
with open('_data/guests.yml', 'w', encoding='utf-8') as f:
    f.write(yaml_content)

print(f"✅ COMPLETE GUESTS DATABASE CREATED!")
print(f"📊 Total guests: {len(real_guests_data)}")
print(f"📁 Saved to: _data/guests.yml")

# Statistics
multiple_episodes = [(name, len(episodes)) for name, episodes in real_guests_data.items() if len(episodes) > 1]
...
print(f"\n📈 STATISTICS:")
print(f"• Total unique guests: {len(real_guests_data)}")
print(f"• Guests with multiple episodes: {len(multiple_episodes)}")
print(f"• Single episode guests: {len(real_guests_data) - len(multiple_episodes)}")
